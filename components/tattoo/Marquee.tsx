...
export default function Marquee({ className = "" }: { className?: string }) {
    ...
    <span className="flex shrink-0 items-center">
      {items.map((s) => (
        <span key={s} className="flex items-center">
          <span className="font-display font-bold uppercase tracking-[0.06em] text-[clamp(1.2rem,2.4vw,1.8rem)] text-volt whitespace-nowrap px-6">
            {s}
          </span>
  ...
  return (
    <div className={`marquee overflow-hidden border-y border-salt/10 bg-void py-4 ${className}`}>
